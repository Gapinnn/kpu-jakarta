...
import { dataBukuPedoman } from "../../../contents/pedoman";
  ...
  return (
    <div className="flex flex-col bg-stone-100 rounded-2xl">
      <div className="py-4 flex flex-col gap-1">
        <h1 className="text-maroon-light text-2xl font-bold">Buku Pedoman</h1>
        <p className="text-stone-900 text-lg">
          Publikasi ini disusun oleh KPU DKI Jakarta untuk memberikan panduan
          dan prosedur yang jelas bagi para pemilih, calon, dan penyelenggara
          pemilu, guna memastikan kelancaran dan keteraturan proses pemilihan
          umum.
        </p>
      </div>
      <div className="mx-auto shadow-xl p-2 py-4 pb-12 border border-gold rounded-2xl grid grid-cols-1 ">
        <div id="bukuPedomanSlider">
          <div className="w-full h-fit slider-container mx-auto">
            <Slider {...settings} className="flex justify-center items-center">
              {dataBukuPedoman.map((item, index) => (
                <Card
                  key={index}
                  title={item.title}
                  date={item.date}
